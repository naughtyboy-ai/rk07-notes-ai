...
function generateTimetable() {
    ...
    let subjectsMap = {
        ...
        "12 arts": ["History","Geo","Pol Sci","English"],
    ...
    let colorClass =
        cls.includes("science") ? "science" :
        cls.includes("commerce") ? "commerce" : "arts";

    let subjects = subjectsMap[cls] || ["Self Study"];

    const week = ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"];
    const slots = ["Morning (8–10)", "Noon (1–3)", "Evening (7–9)"];

    let table = `
    <h3>${cls.toUpperCase()} — Weekly Timetable</h3>
    <table class="timetable-table ${colorClass}">
      <tr>
        <th>Day</th>
        <th>Morning</th>
        <th>Noon</th>
    ...
    week.forEach(day => {
        ...
        slots.forEach(() => {
            table += `<td contenteditable="true">${subjects[i % subjects.length]}</td>`;
            i++;
        });
        table += `</tr>`;
    });
    ...
    table += `</table><br>
      <button class="genBtn" onclick="saveTable()">Save Timetable</button>
    `;

    document.getElementById("result").innerHTML = table;

    saveToLocal(cls, table);
}

function saveToLocal(cls, html){
    localStorage.setItem("timetable", html);
